...
CREATE UNIQUE INDEX idx_tenants_default_single
...
-- ---------------------------------------------------------------
ALTER TABLE domains       ADD COLUMN tenant_id UUID
  REFERENCES tenants(tenant_id)
  DEFAULT '00000000-0000-0000-0000-000000000000' NOT NULL;
ALTER TABLE urls          ADD COLUMN tenant_id UUID
  REFERENCES tenants(tenant_id)
  DEFAULT '00000000-0000-0000-0000-000000000000' NOT NULL;
ALTER TABLE evidence      ADD COLUMN tenant_id UUID
  REFERENCES tenants(tenant_id)
  DEFAULT '00000000-0000-0000-0000-000000000000' NOT NULL;
ALTER TABLE scan_history  ADD COLUMN tenant_id UUID
  REFERENCES tenants(tenant_id)
  DEFAULT '00000000-0000-0000-0000-000000000000' NOT NULL;

CREATE INDEX idx_domains_tenant      ON domains (tenant_id);
CREATE INDEX idx_urls_tenant         ON urls (tenant_id);
CREATE INDEX idx_evidence_tenant     ON evidence (tenant_id);
CREATE INDEX idx_scan_history_tenant ON scan_history (tenant_id);
...
-- ---------------------------------------------------------------
ALTER TABLE urls ADD COLUMN grade        TEXT
  CHECK (grade IN ('A+','A','B','C','D','F','F+'));
ALTER TABLE urls ADD COLUMN page_class   TEXT
  CHECK (page_class IN (
...
ALTER TABLE urls ADD COLUMN page_fingerprint          TEXT;
...
CREATE INDEX idx_urls_page_class   ON urls (page_class) WHERE page_class <> 'generic';
...
CREATE INDEX idx_overrides_tenant_scope_value
  ON overrides (tenant_id, scope, match_value);
CREATE INDEX idx_overrides_user ON overrides (user_id) WHERE user_id IS NOT NULL;
...
CREATE INDEX idx_popup_edges_target ON popup_edges (target_url_hash);
CREATE INDEX idx_popup_edges_recent ON popup_edges (occurred_at DESC);
