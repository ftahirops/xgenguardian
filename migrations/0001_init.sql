...
CREATE INDEX idx_domains_verdict       ON domains (verdict);
CREATE INDEX idx_domains_next_rescan   ON domains (next_rescan_at) WHERE next_rescan_at IS NOT NULL;
CREATE INDEX idx_domains_brand_match   ON domains (brand_match) WHERE brand_match IS NOT NULL;
...
CREATE INDEX idx_urls_verdict ON urls (verdict);
...
CREATE INDEX idx_brand_screenshots_embedding
  ON brand_screenshots USING ivfflat (embedding vector_cosine_ops)
  WITH (lists = 50);
CREATE INDEX idx_brand_screenshots_brand ON brand_screenshots (brand_id);
...
CREATE INDEX idx_evidence_url_hash ON evidence (url_hash);
CREATE INDEX idx_evidence_top_brand ON evidence (visual_top_brand) WHERE visual_top_brand IS NOT NULL;
...
CREATE INDEX idx_scan_history_url_hash ON scan_history (url_hash);
...
CREATE INDEX idx_prescan_queue_pending ON prescan_queue (enqueued_at) WHERE picked_up_at IS NULL;
